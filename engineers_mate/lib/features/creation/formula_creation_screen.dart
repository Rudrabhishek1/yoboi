...
class _FormulaCreationScreenState extends ConsumerState<FormulaCreationScreen> {
  ...
  void _save() async {
    ...
    final variableRegex = RegExp(r'^[a-zA-Z][a-zA-Z0-9]*$');
    for (var variable in variables) {
      if (!variableRegex.hasMatch(variable)) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                "Invalid variable name: $variable. Must start with a letter and contain only letters and numbers.")));
        return;
      }
    }
        ...
        child: Column(
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                  labelText: "Variables (comma separated)", hintText: "m, v"),
            ),
            const SizedBox(height: 16),
            TextField(
              controller: _expressionController,
              decoration: const InputDecoration(
                  labelText: "Equation", hintText: "0.5 * m * v^2"),
            ),
