...
class CustomFormulaData {
  ...
  Formula toFormula() {
    // ⚡ Bolt: Cache parsed expression to avoid expensive repetitive parsing during calculations.
    ...
    return Formula(
      ...
      resultUnit: '',
      calculate: (inputs) {
        // ⚡ Bolt: Use cached parsed expression or parse if not available.
        cachedExpression ??= Parser().parse(expression);
        ContextModel cm = ContextModel();
        for (int i = 0; i < inputLabels.length; i++) {
          cm.bindVariable(Variable(inputLabels[i]), Number(inputs[i]));
        }
        return cachedExpression!.evaluate(EvaluationType.REAL, cm);
      },
    );
  }
}
